...
app.get('/api/generate', async (req, res) => {
  try {
    ...
    if (!promptText && (!imageURLs || imageURLs.length === 0)) {
    ...
    if (imageURLs && imageURLs.length > 0) {
      imageData = await tagImage(imageURLs);
    }
...
function generateText(promptText, imageData) {
  ...
  if (promptText) {
    ...
    return `Generated text based on image tags: ${imageData.map(tag => tag.confidence).join(', ')}`;
  } else {
